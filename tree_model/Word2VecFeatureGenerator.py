...
class Word2VecFeatureGenerator(FeatureGenerator):
    ...
    def process_query(self, df:RDD):
        ...
        CM["Headline_unigram_vec"] = cm_index
        df = df.map(lambda x: x + [preprocess_data(x[CM['Headline']], exclude_stopword=False, stem=False)])
        # df["Headline"].map(lambda x: preprocess_data(x, exclude_stopword=False, stem=False))
        cm_index += 1
        CM["articleBody_unigram_vec"] = cm_index
        df = df.map(lambda x: x + [preprocess_data(x[CM['articleBody']], exclude_stopword=False, stem=False)])
        # df["articleBody"].map(lambda x: preprocess_data(x, exclude_stopword=False, stem=False))
        ...
        Headline_unigram_array = np.array(df.map(lambda x: x[CM['Headline_unigram_vec']]).collect())
        # df['Headline_unigram_vec'].values
        ...
        Body_unigram_array = np.array(df.map(lambda x:x[CM['articleBody_unigram_vec']]).collect())
        # df['articleBody_unigram_vec'].values
