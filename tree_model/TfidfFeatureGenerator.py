...
class TfidfFeatureGenerator(FeatureGenerator):
    ...
    def process_query(self, df:RDD):
        ...
        def cat_text(x):
            res = '%s %s' % (' '.join(x[CM['Headline_unigram']]), ' '.join(x[CM['articleBody_unigram']]))
            return res
        ...
        vec.fit(df.map(lambda x:x[CM["all_text"]]).collect()) # Tf-idf calculated on the combined training + test set
        vocabulary = vec.vocabulary_

        vecH = TfidfVectorizer(ngram_range=(1, 3), max_df=0.8, min_df=2, vocabulary=vocabulary)
        xHeadlineTfidf = vecH.fit_transform(df.map(lambda x: ' '.join(x[CM['Headline_unigram']])).collect()) # use ' '.join(Headline_unigram) instead of Headline since the former is already stemmed
        ...
        xBodyTfidf = vecB.fit_transform(df.map(lambda x: ' '.join(x[CM['articleBody_unigram']])).collect())
...
class TfidfFtXGBoost(TfidfFeatureGenerator):

    def __init__(self, name='tfidfFeatureGenerator'):
        super(TfidfFeatureGenerator, self).__init__(name)

    def read(self, header='train'):
        result = super().read(header)
        return [result[-1]]
